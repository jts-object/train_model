...
class EnvCmd(object):
    ...
    def make_simulation(order_name, file_name, speed_num):
        return {
            ...
            'speed_num': speed_num,
            # speed_num含义: "SPEED"命令时指推演倍速(整型, 1为实际速度, 0为根据硬件自适应加速, 非1数值均按0处理);
    ...
    # 作战飞机指令(共12种)
    @staticmethod
    def make_areapatrol(self_id, px, py, pz, direction, length, width, speed, patrol_time, patrol_mode=0):
        # 区域巡逻(适用于飞机编队)
        validity_pos(px, py)
        ...
        return {
            ...
            'point_y': py,
            'point_z': pz,
            'direction': direction,         # 区域长轴与正北方向角度(整型, 顺时针方向, 单位: 度, [0, 180])
            'length': length,               # 区域长度(整型, 单位: 米)
            'width': width,                 # 区域宽度(整型, 单位: 米)
            'speed': speed,                 # 巡逻速度(浮点型, 单位: 米/秒, 歼击机参考值300m/s, 轰炸机参考值225m/s)
            'patrol_time': patrol_time,     # 巡逻时间(整型, 单位: 秒)
            'patrol_mode': patrol_mode      # 巡逻模式(整型, 0表普通模式, 1表示对角巡逻, 预留接口, 暂无实际效果)
        }

    @staticmethod
    def make_takeoff_areapatrol(airport_id, fly_num, fly_type, px, py, pz, direction, length, width, speed, patrol_time):
        # 起飞区域飞机巡逻
        validity_pos(px, py)
        length, width = validity_area(px, py, length, width, direction)
        return {
            ...
            'fly_type': fly_type,           # 起飞战机类型(11-歼击机; 12-预警机; 13-电子干扰机; 14-无人侦察机；15-轰炸机)
    ...
    @staticmethod
    def make_takeoff_linepatrol(airport_id, fly_num, fly_type, speed, area_id, area_type, point_list):
        # 起飞航线巡逻
        return {
    ...
    @staticmethod
    def make_areahunt(self_id, direction, range, px, py, pz, area_direction, area_length, area_width):
        # 区域突击(适用于轰炸机编队)
        validity_pos(px, py)
        area_length, area_width = validity_area(px, py, area_length, area_width, area_direction)
        range = validity_range(range)
        return {
            'maintype': 'areahunt',
            'self_id': self_id,             # 己方轰炸机编队编号(整型)
            'direction': direction,         # 突击方向, 相对正北方向角度(整型, 逆时针方向, 单位: 度, [0, 360])
            'range': range,                 # 武器发射距离与最大射程的百分比(整型, [1, 100], 距离越近命中率越高, 突防难度也更大)
            'point_x': px,                  # 区域中心点x轴坐标(浮点型, 单位: 米)
            'point_y': py,
            'point_z': pz,
            'area_direct': area_direction,  # 区域长轴与正北方向角度(整型, 顺时针方向, 单位: 度, [0, 180])
            'area_len': area_length,        # 区域长度(整型, 单位: 米)
            'area_wid': area_width          # 区域宽度(整型, 单位: 米)
        }

    @staticmethod
    def make_takeoff_areahunt(airport_id, fly_num, direction, range, px, py, pz, area_direction, area_length, area_width, speed):
        # 起飞区域突击
        validity_pos(px, py)
        area_length, area_width = validity_area(px, py, area_length, area_width, area_direction)
        range = validity_range(range)
        return {
            ...
            'area_len': area_length,        # 区域长度(整型, 单位: 米)
            'area_wid': area_width,         # 区域宽度(整型, 单位: 米)
            'speed': speed                  # 突击速度(浮点型, 单位: 米/秒, 参考速度225m/s)
        }
    ...
    @staticmethod
    def make_takeoff_targethunt(airport_id, fly_num, target_id, direction, range, speed):
        # 起飞目标突击
        range = validity_range(range)
        return {
    ...
    @staticmethod
    def make_airattack(self_id, target_id, type, range=88):
        # 空中拦截(适用于在空歼击机平台)
        range = validity_range(range)
        return {
            'maintype': 'airattack',
            ...
            'target_id': target_id,         # 敌方平台编号(整型)
            'type': type,                   # 拦截的引导方法(整型, 0/1, 预留接口暂无实际效果)
            'range': range                  # 发弹距离与最大射程的百分比(整型, [1, 100])
        }
    ...
    def make_ground_setdirection(self_id, direction):
        ...
        return {
            ...
            'self_id': self_id,             # 己方地防编队编号(整型)
            'direction': direction          # 防御方向, 与正北方向夹角(整型, 逆时针方向, 单位: 度, [0, 360])
        }
    ...
    def make_ship_movedeploy(self_id, px, py, pz, direction, radar_state):
        ...
        return {
            ...
            'point_y': py,
            'point_z': pz,
            'direction': direction,         # 防御方向, 与正北方向夹角(可给任意值, 因为护卫舰是360度防空的)
            'radar_state': radar_state      # 雷达开关机状态(默认为1)
        }

    @staticmethod
    def make_ship_areapatrol(self_id, px, py, pz, direction, length, width, speed, patrol_time, patrol_mode=0):
        # 舰船区域巡逻防空(适用于己方护卫舰编队)
        validity_pos(px, py)
        length, width = validity_area(px, py, length, width, direction)
        return {
    ...
    # 预警机(共5种)
    @staticmethod
    def make_awcs_areapatrol(self_id, px, py, pz, direction, length, width, speed, patrol_time, patrol_mode=0):
        # 预警机区域巡逻侦察(适用于预警机编队)
        validity_pos(px, py)
        length, width = validity_area(px, py, length, width, direction)
        return {
            ...
            'length': length,               # 区域长度(整型, 单位: 米)
            'width': width,                 # 区域宽度(整型, 单位: 米)
            'speed': speed,                 # 巡逻速度(浮点型, 单位: 米/秒, 参考值225m/s)
            'patrol_time': patrol_time,     # 巡逻时间(整型, 单位: 秒)
            'patrol_mode': patrol_mode      # 巡逻模式(整型, 0表普通模式, 1表示对角巡逻, 预留接口, 暂无实际效果)
        }
    ...
    # 干扰机(共5种)
    @staticmethod
    def make_disturb_areapatrol(self_id, px, py, pz, direction, length, width, speed, disturb_time):
        # 区域干扰(适用于干扰机编队)
        validity_pos(px, py)
        length, width = validity_area(px, py, length, width, direction)
        return {
    ...
    @staticmethod
    def make_disturb_linepatrol(self_id, speed, area_id, area_type, point_list):
        # 航线干扰(适用于干扰机编队)
        return {
    ...
    # 无人侦察机(共3种)
    @staticmethod
    def make_uav_areapatrol(self_id, px, py, pz, direction, length, width, speed, patrol_time, patrol_mode=0):
        # 区域侦察(适用于侦察机编队)
        validity_pos(px, py)
        length, width = validity_area(px, py, length, width, direction)
        return {
...
def validity_area(pos_x, pos_y, length, width, direction):
    ...
    # 矩形区域左下角坐标
    ld_x = pos_x - 0.5*length*math.sin(direction) - 0.5*width*math.cos(direction)
    ld_y = pos_y - 0.5*length*math.cos(direction) + 0.5*width*math.sin(direction)
    # 矩形区域右下角坐标
    rd_x = pos_x - 0.5*length*math.sin(direction) + 0.5*width*math.cos(direction)
    rd_y = pos_y - 0.5*length*math.cos(direction) - 0.5*width*math.sin(direction)
    # 矩形区域左上角坐标
    lt_x = pos_x + 0.5*length*math.sin(direction) - 0.5*width*math.cos(direction)
    lt_y = pos_y + 0.5*length*math.cos(direction) + 0.5*width*math.sin(direction)
    # 矩形区域右上角坐标
    rt_x = pos_x + 0.5*length*math.sin(direction) + 0.5*width*math.cos(direction)
    rt_y = pos_y + 0.5*length*math.cos(direction) - 0.5*width*math.sin(direction)
    # 越界检查与修正
    diag = math.sqrt(length**2 + width**2)  # 矩形对角线长度
    ratio1 = 1
    ratio2 = 1
    # print("矩形左下角坐标({},{}), 右下角坐标({},{}), 左上角坐标({},{}), 右上角坐标({},{})".
    #       format(ld_x, ld_y, rd_x, rd_y, lt_x, lt_y, rt_x, rt_y))
    if ld_x < X_Min or rt_x > X_Max or lt_x > X_Max or rd_x < X_Min:
        # 左下角x坐标越界, 右上角x坐标越界, 对应的direction区间(0, 90]
        # 左上角x坐标越界, 或者右下角x坐标越界, 对应direction区间[90, 180)
        # 这四种是x轴坐标越界情况
        # 中心点距离最近的x轴边界距离(并检验中心点是否合理(距离边界至少2公里, 否则没法转弯或者一直大角度机动))
        margin = min(pos_x-X_Min, X_Max-pos_x)
        assert margin >= 1000
        # 矩形中心点到越界顶点距离在x轴上的投影长度proj为
        # proj = 0.5*diag*math.sin(direction+theta) if direction+theta <= 90 else 0.5*diag*math.sin(180-direction-theta)
        # 确定长宽缩小比例
        ratio1 = 2*margin / diag
    if lt_y > Y_Max or rd_y < Y_Min or rt_y < Y_Min or ld_y > Y_Max:
        # 左上角y坐标越界, 或者右下角y坐标越界, 对应direction区间为(0, 90]
        # 右上角y坐标越界, 或者左下角y坐标越界, 对应direction区间[90, 180)
        # 这四种是y轴坐标越界情况
        # 中心点距离最近的y轴边界距离(并检验中心点是否合理(距离边界至少2公里, 否则没法转弯或者一直大角度机动))
        margin = min(pos_y-Y_Min, Y_Max-pos_y)
        assert margin >= 1000
        # 矩形中心点到越界顶点距离在y轴上的投影长度proj为
        # proj = 0.5*diag*math.cos(direction+theta) if direction+theta <= 90 else 0.5*diag*math.cos(180-direction-theta)
        # 确定长宽缩小比例
        ratio2 = 2*margin / diag
    width = width * min(ratio1, ratio2)
    length = length * min(ratio1, ratio2)
    return length, width


# 检查突击类指令的发弹距离百分比设置是否越界
def validity_range(att_range):
    # 发弹距离百分比检查(0-100]
    assert isinstance(att_range, int)
    if att_range <= 1:
        att_range = 1
    if att_range >= 100:
        att_range = 100
...
# 检查航线类指令的航路点设是否出界
def validity_point(point_list):
    # 这里只检查x, y坐标, z轴范围限高设置为18000
    for point in point_list:
        point.x = sorted([X_Min, point.x, X_Max])[1]
        point.y = sorted([Y_Min, point.y, Y_Max])[1]
        point.z = sorted([0, point.z, 18000])[1]
